...
def fetch_realtime_tencent(code: str) -> dict:
    ...
    return {
        ...
        "amount_yi": float(parts[37]) if len(parts) > 37 and parts[37] else 0,
...
def main():
    ...
    daily = kline
    print(f"  K线数据范围: {daily[0]['date']} ~ {daily[-1]['date']}")
    ...
    report = {
        ...
        "summary": {
            ...
            "change_1w": round((today.get("close", 0) - week_ago.get("close", 0)) / week_ago.get("close", 1) * 100, 1) if week_ago.get("close", 0) > 0 else 0,
            "change_1m": round((today.get("close", 0) - month_ago.get("close", 0)) / month_ago.get("close", 1) * 100, 1) if month_ago.get("close", 0) > 0 else 0,
        },
        "realtime": realtime,
        "daily": daily,
