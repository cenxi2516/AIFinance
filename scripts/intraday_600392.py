#!/usr/bin/env python3
"""
盘中实时交易信号系统 V1.3 — 盛和资源(600392) 七维分析
日期: 2026-07-13 盘中实时
...
def fetch_minute_fund_flow_stock(code):
    """
    个股分钟级资金流 (东财 push2 API)
    返回每分钟主力净流入/超大单/大单/中单/小单 (累计值转增量)

    字段映射 (push2 fflow/kline/get):
      f52=主力净流入(累计), f53=小单(累计), f54=中单(累计), f55=大单(累计), f56=超大单(累计)
      主力 = 大单 + 超大单  ✓ 已验证
    """
    ...
    try:
        ...
        for line in klines:
            ...
            if len(parts) >= 6:
                try:
                    cum_data.append({
                        "time": parts[0],
                        "main_net_cum": float(parts[1]) if parts[1] != '-' else 0,    # f52
                        "small_net_cum": float(parts[2]) if parts[2] != '-' else 0,   # f53
                        "mid_net_cum": float(parts[3]) if parts[3] != '-' else 0,     # f54
                        "large_net_cum": float(parts[4]) if parts[4] != '-' else 0,   # f55
                        "super_net_cum": float(parts[5]) if parts[5] != '-' else 0,   # f56
                    })
        ...
        return {"error": f"资金流获取失败: {e}", "minutes": []}
...
RE_SECTORS = {
    ...
    "BK0488": "稀缺资源",
    "BK0502": "新材料",
    "BK0575": "新能源车",  # 稀土是新能源车永磁电机上游
}

def fetch_sector_sentiment():
    """
    拉取全量概念板块(m:90+t:3)+行业板块(m:90+t:2) → 筛选盛和资源核心板块。
    有色金属/小金属/新材料是行业板块(t:2)，稀土永磁/稀缺资源/新能源车是概念板块(t:3)。
    """
    all_sectors = {}
    ...
    blocks = []
    for bk_code, bk_name in RE_SECTORS.items():
...
def fetch_intraday_news(keywords):
    ...
    for kw in keywords:
        ...
        bullish_kw = ["增长","突破","中标","扩产","获批","回购","增持","超预期","利好","扶持","涨价","反弹"]
        bearish_kw = ["减持","亏损","下滑","调查","处罚","诉讼","违约","制裁","限制","关税","降价","暴跌"]
        for a in articles:
...
def fetch_north_bound():
    """
    北向资金实时流向。优先用同花顺hsgtApi，失败则尝试东财备用接口。
    当前状态(2026-07): 同花顺hsgtApi DNS不可用，切换到东财接口。
    """
    # 方法1: 尝试东财北向资金接口
    try:
    ...
    return {
        ...
        "data_basis": "北向资金API暂不可用(同花顺DNS失败,东财无实时数据)",
        "source": "无",
        "status": "unavailable",
    }
...
def fetch_market_breadth():
    ...
    else: bl = "⚠️ 数据缺失"; bs = 30  # API全部失败时中性处理
...
# 主流程
# ============================================================
def _make_placeholder_flow(code, quote):
    """当 push2 分钟资金流不可用时，生成占位 flow 结构，下行分析降级为量价模式"""
    from datetime import datetime
    now = datetime.now()
    return {
        "minutes": [],
        "summary": {
            "total_main_net_wan": 0, "total_super_net_wan": 0, "total_large_net_wan": 0,
            "total_mid_net_wan": 0, "total_small_net_wan": 0,
            "data_points": 1, "first_time": now.strftime("%H:%M"), "last_time": now.strftime("%H:%M"),
        },
        "trend": {"direction": "无数据", "acceleration": "无数据",
                  "first_half_slope_wan_per_min": 0, "second_half_slope_wan_per_min": 0},
        "_fund_flow_available": False,
    }
...
def main():
    code = "600392"
    print("═"*70)
    print(f"  盘中实时交易信号系统 V1.3 — 盛和资源({code})")
    print(f"  时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} (周一)")
    print("═"*70)
    ...
    fund_flow_available = "error" not in raw_flow
    if not fund_flow_available:
        print(f"  ⚠️ push2 分钟资金流不可用: {raw_flow.get('error', '未知')[:80]}")
        print(f"  ⚠️ 降级为实时行情量价分析模式")
        flow = _make_placeholder_flow(code, quote)
    else:
    ...
    if fund_flow_available:
        ...
        print(f"  量价代理: 涨跌{quote['change_pct']:+.2f}% 换手{quote['turnover_pct']:.2f}% 量比{quote['vol_ratio']:.2f}")
    ...
    if fund_flow_available:
        ...
        print(f"  博弈: 无分钟资金流 → 仅量价参考")
        # 标记 fund flow 维度不可用
        flow["_fund_flow_available"] = False

    # [3/6] 板块情绪 + 新闻
    print("\n[3/6] 拉取板块情绪 + 新闻...")
    sector = fetch_sector_sentiment()
    print(f"  稀土/有色板块情绪: {sector['sentiment_score']:.0f}/100 {sector['level']}")
    ...
    news = fetch_intraday_news(["盛和资源", "稀土", "有色金属", "永磁"])
