...
def main():
    ...
    print(f"╔══════════════════════════════════════════════════════════════╗")
    print(f"║  fund-flow-predictor V1.4 — 买入安全区间研判                   ║")
    print(f"║  标的: 盛和资源 ({CODE})                                          ║")
    print(f"║  日期: {datetime.now().strftime('%Y-%m-%d')}                                                  ║")
    print(f"╚══════════════════════════════════════════════════════════════╝")
    print()

    # ━━ Step 1: 拉取核心数据 ━━
    print("[1/7] 拉取腾讯日K线(qfq)...")
    ...
    print(f"  名称: {realtime.get('name', '?')} | 价格: {realtime.get('price', 0):.2f}")

    print("[3/7] 拉取个股资金流(push2his)...")
    fund_flow = fetch_fund_flow(CODE)
    if fund_flow:
        merged = merge_fund_flow(daily, fund_flow)
        print(f"  合并 {merged} 日资金流到K线")
    else:
        print(f"  无资金流数据，使用量价分析模式")
    ...
    if concept_flow.get("available"):
        print(f"  板块: {concept_flow.get('board_code', '?')} | {concept_flow.get('stock_count', 0)}只成分股 | 主力合计{concept_flow.get('total_main_net_yi', 0):+.2f}亿")
    else:
        print(f"  ⚠️ {concept_flow.get('reason', '概念板块不可用')}")
    ...
    if margin_data.get("available"):
        ...
        print(f"  ⚠️ {margin_data.get('reason', '融资融券不可用')}")
    ...
    report = {
        ...
        "target": CODE,
        "target_name": realtime.get("name", "盛和资源"),
        ...
        "summary": {
            ...
            "change_1w": round(
                (today.get("close", 0) - week_ago.get("close", 0)) / week_ago.get("close", 1) * 100, 1
            ),
            "change_1m": round(
                (today.get("close", 0) - month_ago.get("close", 0)) / month_ago.get("close", 1) * 100, 1
            ),
        },
        "realtime": realtime,
        "daily": daily,
    ...
    # 保存
    output_dir = f"src/资金预判/{datetime.now().strftime('%Y-%m-%d')}-盛和资源-安全评估"
