...
STOCK_CONCEPT_MAP = {
    "600176": "BK0546",  # 中国巨石 → 玻璃玻纤
    "600879": "BK0963",  # 航天电子 → 商业航天
}
...
def fetch_margin_data(code: str) -> dict:
    """获取个股融资融券数据 (API: datacenter-web, filter用SCODE)"""
    try:
...
def detect_volume_stabilization(daily: list[dict], track: str = "B") -> dict:
    ...
    vols_20 = [d["volume"] for d in daily[-20:]]
...
def calc_safety_score(safety_signals: dict, danger_signals: dict, daily: list[dict],
    ...
    """安全区间综合评分 (0-100) — V1.4 track-aware + 融资融券修正"""
    score = 50
    adjustment_details = []
...
def main():
    ...
    print("=" * 70)
    print(f"  fund-flow-predictor V1.4 — {TARGET_NAME}({TARGET}) 安全评估")
    print("=" * 70)

    # ━━ 0. 实盘行情确认 ━━
    print("\n[0/6] 实盘行情确认 (腾讯+新浪双源验证)...")
    realtime = fetch_realtime_with_validation(TARGET)
    print(f"  标的: {realtime.get('name', '?')} | 现价: {realtime.get('price', 0):.2f}")
    ...
    # ━━ 4. 拉取新闻 + 辅助数据 ━━
    print("\n[4/6] 拉取新闻+行业资金流+概念板块+融资融券...")
    news = fetch_news(TARGET)
    print(f"  新闻: {len(news)}条")
    concept_flow = fetch_concept_board_flow(TARGET)
    if concept_flow.get("available"):
        print(f"  概念板块: {concept_flow.get('board_code','')} | 成分股{concept_flow.get('stock_count',0)}只 "
              f"| 主力合计{concept_flow.get('total_main_net_yi',0):+.2f}亿")
    else:
        print(f"  概念板块: 不可取 ({concept_flow.get('reason','')})")
    margin_data = fetch_margin_data(TARGET)
    if margin_data.get("available"):
        print(f"  融资融券: 余额{margin_data.get('latest_balance_yi',0):.2f}亿 | 趋势{margin_data.get('trend','')}")
    else:
        print(f"  融资融券: 不可取 ({margin_data.get('reason','')})")
    ...
    for key, sig in safety_signals.items():
        status = "🟢触发" if sig.get("triggered") else "⚪未触发"
        print(f"  {sig.get('signal_name', key)}: {status}")

    for key, sig in danger_signals.items():
        if sig.get("triggered"):
            print(f"  {sig.get('signal_name', key)}: 🔴触发! 等级:{sig.get('danger_level','')}")
    ...
    # 更新最后一日数据
    _update_last_day(daily, realtime)
...
def _update_last_day(daily: list[dict], realtime: dict):
    """用实时行情更新 daily 最后一天"""
