...
标的: 南山铝业 (600219)
日期: 2026-07-13
类型: 上海主板个股 → 尝试A轨(资金流), 不可用则降级B轨(量价)
"""
...
def calc_safety_score(safety_signals: dict, danger_signals: dict, daily: list[dict],
    ...
    if track == "A":
        if safety_signals.get("f1_exhaustion", {}).get("triggered"):
            score += 15; adjustment_details.append(("F1 主力流出衰竭", +15))
        if safety_signals.get("f2_institution_return", {}).get("triggered"):
            score += 12; adjustment_details.append(("F2 机构试探回流", +12))
        if safety_signals.get("f3_retail_exhaustion", {}).get("triggered"):
            score += 10; adjustment_details.append(("F3 散户恐慌出尽", +10))
        if safety_signals.get("f8_volume_stabilization", {}).get("triggered"):
            score += 8; adjustment_details.append(("F8 缩量止跌", +8))
    else:
        if safety_signals.get("f1_exhaustion", {}).get("triggered"):
            score += 12; adjustment_details.append(("F1 卖压衰竭", +12))
        if safety_signals.get("f2_institution_return", {}).get("triggered"):
            score += 10; adjustment_details.append(("F2 放量反弹回流", +10))
        if safety_signals.get("f3_retail_exhaustion", {}).get("triggered"):
    ...
    if track == "A":
        if danger_signals.get("f4_accelerating", {}).get("triggered"):
            score -= 22; adjustment_details.append(("F4 主力加速流出", -22))
        if danger_signals.get("f5_top_distribution", {}).get("triggered"):
            score -= 20; adjustment_details.append(("F5 顶部派发", -20))
    else:
        if danger_signals.get("f4_accelerating", {}).get("triggered"):
            score -= 20; adjustment_details.append(("F4 加速放量下跌", -20))
        if danger_signals.get("f5_top_distribution", {}).get("triggered"):
            score -= 18; adjustment_details.append(("F5 高位放量逆转", -18))
    ...
    if len(daily) >= 20:
        ...
        if deviation > 15:
            score -= 5; adjustment_details.append((f"偏离20MA+{deviation:.0f}%(超买)", -5))
        elif deviation < -10:
            score += 5; adjustment_details.append((f"偏离20MA{deviation:.0f}%(超卖)", +5))
    ...
    if len(daily) >= 20:
        ...
        if vol_ratio > 3.0 and daily[-1]["change_pct"] < -3:
            score -= 8; adjustment_details.append((f"恐慌放量({vol_ratio:.1f}x均量+大跌)", -8))
        elif vol_ratio > 2.0 and daily[-1]["change_pct"] < -2:
            score -= 5; adjustment_details.append((f"放量下跌({vol_ratio:.1f}x均量)", -5))
        elif vol_ratio < 0.4 and abs(daily[-1]["change_pct"]) < 1:
            score += 3; adjustment_details.append(("缩量止跌企稳", +3))
    ...
    if margin_data and margin_data.get("available"):
        ...
        if trend == "下降" and net_5d < -1:
            score -= 7; adjustment_details.append(("融资去杠杆(趋势下降+净卖出>1亿)", -7))
        elif trend == "下降" and net_5d < 0:
            score -= 4; adjustment_details.append(("融资温和去杠杆", -4))
        elif trend == "上升" and net_5d > 2:
            score += 5; adjustment_details.append(("融资加杠杆(趋势上升+净买入>2亿)", +5))
        elif trend == "上升" and net_5d > 0:
            score += 3; adjustment_details.append(("融资温和加杠杆", +3))

    # 概念板块龙头修正
    if concept_flow and concept_flow.get("available"):
        rank = concept_flow.get("target_rank", 0)
        tf = concept_flow.get("target_flow", {})
        main_yi = tf.get("main_net_yi", 0) if tf else 0
        if rank == 1 and main_yi > 0:
            score += 4; adjustment_details.append(("概念板块主力流入第1名", +4))
...
def generate_timing(safety_score: dict, daily: list[dict]) -> dict:
    ...
    entry_conditions = [
        ...
        {"condition": "≥2个安全信号触发", "met": len(safety_score["positive_signals"]) >= 2,
         "current": f"{len(safety_score['positive_signals'])}个", "weight": "建议"},
    ...
    exit_conditions = [
        ...
        {"condition": f"🔴 任何高危信号触发 (F4/F5/F7)", "triggered": neg_count > 0,
    ...
    verdict = ("✅ 买入参考条件全部满足，可考虑入场" if all_met else
               ("⚠️ 必要条件满足但增强信号不足，可小仓试探" if must_met else
                "❌ 买入条件不满足，继续等待"))
    return {
        "entry": {"ready": must_met, "ready_enhanced": all_met, "conditions": entry_conditions, "verdict": verdict},
...
def _print_report(report: dict):
    ...
    print(f"  今开: {summary.get('open', 0):.2f} | 最高: {summary.get('high', 0):.2f} | "
          f"最低: {summary.get('low', 0):.2f} | 昨收: {summary.get('last_close', 0):.2f}")
    print(f"  成交额: {summary.get('amount_yi', 0):.2f}亿 | 换手率: {summary.get('turnover_pct', 0):.2f}% | "
          f"振幅: {summary.get('amplitude_pct', 0):.2f}%")
    print(f"  总市值: {summary.get('mcap_yi', 0):.1f}亿 | PE(TTM): {summary.get('pe_ttm', 0):.1f} | "
          f"PB: {summary.get('pb', 0):.2f}")
    print(f"  1周涨跌: {summary.get('change_1w', 0):+.1f}% | "
          f"1月涨跌: {summary.get('change_1m', 0):+.1f}%")

    # 数据来源质量
    validation = report.get("realtime", {}).get("_validation", {})
    if validation:
    ...
    if margin_data and margin_data.get("available"):
        ...
        reason = margin_data.get("reason", "") if margin_data else ""
...
def _save_markdown(report: dict, path: str):
    ...
    lines.append(f"")
    lines.append(f"**标的**: {report.get('target_name', report['target'])} ({report['target']}) | **类型**: {report['target_type']} | **信号轨**: {report.get('track_label', '')}")
    ...
    lines.append(f"| 今开/最高/最低 | {summary.get('open', 0):.2f} / {summary.get('high', 0):.2f} / {summary.get('low', 0):.2f} |")
    lines.append(f"| 成交额/换手/振幅 | {summary.get('amount_yi', 0):.2f}亿 / {summary.get('turnover_pct', 0):.2f}% / {summary.get('amplitude_pct', 0):.2f}% |")
    lines.append(f"| 总市值/PE(TTM)/PB | {summary.get('mcap_yi', 0):.1f}亿 / {summary.get('pe_ttm', 0):.1f} / {summary.get('pb', 0):.2f} |")
    ...
    if margin_md and margin_md.get("available"):
        balance_md = margin_md.get("latest_balance_yi", 0)
        trend_md = margin_md.get("trend", "平稳")
        net_5d_md = margin_md.get("net_flow_5d_yi", 0)
    ...
    labels_md = {"no_new_low": "不再创新低", "narrow_range": "振幅 < 5%", "flow_normal": "流出/量能正常",
                 "lows_rising": "最低价抬高", "above_ma5": "站上5日均线"}
...
def main():
    code = "600219"
    target_name = "南山铝业"
    print(f"[识别] 上海主板个股: {target_name} ({code})")

    # ━━ 1. 拉取K线 ━━
    print("[1/7] 拉取腾讯日K线(qfq,60日)...")
    kline = fetch_kline_tencent(code, days=60)
    print(f"  获取 {len(kline)} 个交易日K线")

    # ━━ 2. 拉取实时行情 ━━
    print("[2/7] 拉取腾讯+新浪实时行情(交叉验证)...")
    realtime = fetch_realtime_with_validation(code)
    print(f"  名称: {realtime.get('name', '未知')} | 价格: {realtime.get('price', 0)}")

    # ━━ 3. 尝试拉取资金流 ━━
    print("[3/7] 拉取个股资金流(push2his)...")
    fund_flow = fetch_fund_flow(code)
    if fund_flow:
        merged = merge_fund_flow(kline, fund_flow)
        print(f"  合并资金流: {merged}日匹配")
    else:
        print(f"  资金流不可用，使用量价分析模式")

    track = determine_track(kline)
    track_label = "A轨(个股-资金流+量价)" if track == "A" else "B轨(个股-量价分析-无资金流维度)"
    print(f"  信号轨选择: {track_label}")
    ...
    # ━━ 4. 拉取新闻 ━━
    print("[4/7] 拉取新闻...")
    news = fetch_news("南山铝业")
    print(f"  获取 {len(news)} 条新闻")

    # ━━ 5. V1.4新增: 概念板块资金流 + 融资融券 ━━
    print("[5/7] 拉取概念板块资金流...")
    ...
    if concept_flow.get("available"):
        ...
        tf = concept_flow.get("target_flow", {})
        if tf:
            print(f"  南山铝业排名: {concept_flow.get('target_rank',0)}/{concept_flow.get('target_rank_total',0)} | "
                  f"主力净流{tf.get('main_net_yi',0):+.2f}亿")
    else:
