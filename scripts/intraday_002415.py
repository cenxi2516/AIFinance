#!/usr/bin/env python3
"""
盘中实时交易信号系统 V1.3 — 海康威视(002415) 七维分析
日期: 2026-07-13 盘中
...
def fetch_minute_fund_flow(code):
    ...
    try:
        ...
        if len(deltas) >= 10:
            ...
            first_half_deltas = deltas[1:mid_point]
...
# ====== Layer 3: 板块情绪 ======
# 海康威视核心概念板块（安防+AI+机器视觉+物联网+大数据+国产替代）
HIKVISION_SECTORS = {
    "BK0735": "安防设备",
    "BK0800": "人工智能",
    "BK0893": "机器视觉",
...
def fetch_hikvision_sector_sentiment():
    """拉取全量概念板块 → 筛选海康威视核心板块"""
    try:
    ...
    # 筛选海康核心板块
    sentiment_blocks = []
    for bk_code, bk_name in HIKVISION_SECTORS.items():
...
def print_report(quote, flow, mootdx_flow, sector, news, nb, ms, mb, scenarios, signals):
    print()
    print("═" * 70)
    print(f"  📊 盘中实时交易信号 V1.3: 海康威视(002415)")
    ...
    for sc in scenarios.get("scenarios", []):
        ...
        for c in sc.get("conditions", []):
            ...
            print(f"     {status} {c['condition']}: 阈值{c['threshold']}, 当前={c['actual']} (权重{c['weight']})")
...
def main():
    print(f"\n{'='*60}")
    print(f"  海康威视(002415) 盘中实时交易信号 V1.3")
    ...
    # 3. 板块情绪
    print("\n[3/6] 拉取安防/AI/机器视觉板块情绪...")
    sector = fetch_hikvision_sector_sentiment()
    if "error" in sector:
