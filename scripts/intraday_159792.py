#!/usr/bin/env python3
"""
盘中实时交易信号系统 V1.2 — 港股通互联网ETF富国(159792) 七维分析
日期: 2026-07-13 盘中实时
数据源: mootdx(分时买卖力道) + 腾讯(行情/K线) + 东财push2(板块/广度) + 同花顺(北向)

港股通互联网ETF 跟踪中证港股通互联网指数(931637)，重仓: 腾讯/阿里/小米/美团/快手/京东
适配板块: 互联网服务/中概互联/数字经济/云计算/人工智能
"""
...
# ============================================================
def fetch_minute_data_mootdx(code, date='20260711'):
    """
    拉取指定日期的分时数据。
    盘中: date=今天，用最新交易日
    """
...
def fetch_internet_sector_sentiment():
    """
    拉取全量概念板块 -> 筛选港股通互联网相关板块
    通过名称关键词匹配
    """
    ...
    # 关键词过滤
    blocks = []
    for bk_code, sd in all_sectors.items():
        name = sd["name"]
        if any(kw in name for kw in INTERNET_KEYWORDS):
            blocks.append(sd)

    # 如果关键词匹配不到（罕见），取全市场前8个有资金流的板块
    if not blocks:
        sorted_by_flow = sorted(all_sectors.values(),
                                key=lambda x: abs(x["main_net_wan"]), reverse=True)
        blocks = sorted_by_flow[:8]
        print(f"  [WARN] 关键词未匹配到板块，使用全市场TOP8")

    # 按主力资金排序取TOP8
    blocks = sorted(blocks, key=lambda x: abs(x["main_net_wan"]), reverse=True)[:8]

    inflow = sum(1 for b in blocks if b["main_net_wan"] > 0)
    total = len(blocks)
    avg_chg = sum(b["change_pct"] for b in blocks) / max(total, 1)
...
def fetch_intraday_news(keywords):
    ...
    for kw in keywords:
        ...
        bullish_kw = ["增长","突破","中标","扩产","获批","回购","增持","超预期","利好","扶持","补贴","反弹"]
        bearish_kw = ["减持","亏损","下滑","调查","处罚","诉讼","违约","制裁","限制","关税","暴跌","崩盘"]
        for a in articles:
            title = re.sub(r'<[^>]+>', '', a.get("title", ""))
            if a.get("date", "").startswith(today) or True:  # 也显示近期新闻
                sentiment = "neutral"
    ...
    today = datetime.now().strftime("%Y-%m-%d")
    today_news = [h for h in unique if h["time"].startswith(today)]

    return {
        "today_count": len(today_news), "highlights": unique[:15],
        "bullish_count": sum(1 for h in today_news if h["sentiment"]=="bullish"),
        "bearish_count": sum(1 for h in today_news if h["sentiment"]=="bearish"),
    }
...
def fetch_market_breadth():
    ...
    try:
        r = requests.get("https://push2ex.eastmoney.com/getTopicZTPool",
                        params={"ut": "7eea3edcaed734bea9cbfc24409ed989", "Pageindex": 0,
                                "pagesize": 500, "sort": "fbt:asc",
...
def generate_scenarios(quote, flow, sector, news, nb, ms, mb):
    ...
    scenarios.append({
        ...
        "failure_conditions": ["港股突发大跌", "板块情绪骤降"],
    })
...
def generate_signals(scenarios, quote, flow, sector, nb, ms, mb):
    ...
    if not sell_signals:
        sell_signals.append({"level": "⚪ 暂不需卖出",
            "data_evidence": [
                {"item": "买方力道", "value": f"{buy_ratio:.1f}%", "threshold": ">50%保持", "source": "mootdx", "met": buy_ratio > 50}],
            "action": "继续持有观察",
            "principle": "保持警觉"})
    ...
    if "情景A" in pn and pp >= 45:
        hold = {"level": "✅ 可继续持有", "reason": f"反弹概率{pp:.0f}%, 买方力道{buy_ratio:.1f}%",
                "watch_points": ["买力是否转弱", "情绪是否骤降", "港股是否异动"]}
    elif "情景B" in pn:
...
def print_report(quote, klines, flow, sector, news, nb, ms, mb, scenarios, signals, code):
    ...
    print(f"  跟踪指数: 中证港股通互联网(931637) — 重仓:腾讯/阿里/小米/美团")
    print("═"*70)
    ...
    print(f"  最新价: {quote.get('price',0):.3f}  涨跌: {quote.get('change_pct',0):+.2f}%  振幅: {quote.get('amplitude',0):.2f}%")
    ...
    print(f"  全市场板块: {sector.get('all_sectors_count', 0)}个, 匹配互联网相关: {sector['total_blocks']}个")
    ...
    print(f"\n{'─'*60}\n  📰 四、消息面（港股通互联网主题）\n{'─'*60}")
    print(f"  相关新闻: {news['today_count']}条今日 (利好{news['bullish_count']}/利空{news['bearish_count']})")
    for h in (news.get("highlights") or [])[:10]:
    ...
    print(f"\n{'═'*70}")
    print(f"  ⚠️ 研究声明:")
    print(f"  1. 买卖力道基于mootdx分时数据推导，不构成投资建议")
    print(f"  2. 港股通互联网ETF(159792)跟踪中证港股通互联网指数(931637)")
    print(f"  3. 底层资产为港股，受港股交易时段(9:30-12:00/13:00-16:00)影响")
...
def main():
    ...
    print("═"*70)
    print("  盘中实时交易信号系统 V1.2 — 港股通互联网ETF富国(159792)")
    print(f"  日期: {now.strftime('%Y-%m-%d')} ({'周一' if now.weekday()==0 else ['周二','周三','周四','周五','周六','周日'][now.weekday()-1]}) {now.strftime('%H:%M')}")
    print(f"  跟踪: 中证港股通互联网指数(931637)")
    print(f"  重仓: 腾讯/阿里/小米/美团/快手/京东")
    print("═"*70)
    ...
    weekday = now.weekday()
    if weekday >= 5:
        print("\n⚠️ 今日为周末，A股休市。将使用最近交易日数据。")
    elif 9*60+30 <= minutes <= 11*60+30:
        print(f"\n📍 上午交易时段 ({now.strftime('%H:%M')})，开盘{minutes-9*60-30}分钟")
    elif 13*60 <= minutes <= 15*60:
        print(f"\n📍 下午交易时段 ({now.strftime('%H:%M')})，已交易{minutes-9*60-30-90}分钟")
    elif minutes < 9*60+30:
        print(f"\n📍 盘前阶段 ({now.strftime('%H:%M')})，距开盘还有{9*60+30-minutes}分钟")
    else:
        print(f"\n📍 盘后阶段，展示最近交易日完整数据")

    print("\n[1/6] 实时行情 + 日K线...")
    quote = fetch_tencent_quote(code)
    print(f"  {quote.get('name','?')}({code}) 最新{quote.get('price',0):.3f} 涨跌{quote.get('change_pct',0):+.2f}%  振幅{quote.get('amplitude',0):.2f}%")
    ...
    # 分时数据: 尝试今天，失败则用最近交易日
    print("\n[2/6] 分钟级买卖力道...")
    md = fetch_minute_data_mootdx(code, date_str)
    if not md:
        # 尝试上一个交易日
        print(f"  今天({date_str})无分时数据，尝试上一交易日...")
        md = fetch_minute_data_mootdx(code, '20260711')  # 周五
    if not md:
        md = fetch_minute_data_mootdx(code, '20260710')
    print(f"  分时数据: {len(md)}条")
    flow = analyze_minute_flow(md)
    if "error" in flow:
        print(f"  ⚠️ {flow['error']}")
        # 用虚拟数据避免崩溃
        flow = {"minutes": [], "summary": {"total_buy_vol": 0, "total_sell_vol": 0,
            "total_net_vol": 0, "buy_ratio_pct": 50, "data_points": 0,
            "first_time": "", "last_time": ""},
            "trend": {"direction": "数据不足", "acceleration": "数据不足",
                      "first_half_slope": 0, "second_half_slope": 0, "turning_points": []}}
    else:
        fs = flow["summary"]; ft = flow["trend"]
    ...
    print("\n[3/6] 互联网板块情绪 + 新闻...")
    sector = fetch_internet_sector_sentiment()
    print(f"  互联网板块情绪: {sector['sentiment_score']:.0f}/100 {sector['level']}")
    print(f"  全市场板块: {sector.get('all_sectors_count', 0)}个")
    print(f"  匹配互联网相关: {len(sector.get('blocks', []))}个板块")
    for bk in sector.get("blocks", []):
        print(f"    {bk['name']}: 主力{bk['main_net_wan']:+.0f}万 涨跌{bk['change_pct']:+.2f}%")
    news = fetch_intraday_news(["港股通互联网", "中概互联", "腾讯", "阿里", "美团", "小米", "数字经济"])
    print(f"  新闻: {news['today_count']}条今日 (利好{news['bullish_count']}/利空{news['bearish_count']})")
    ...
    dir_name = f"{date_dir}-港股通互联网ETF富国-盘中信号"
    dir_path = os.path.join("src/盘中交易信号", dir_name)
    os.makedirs(dir_path, exist_ok=True)

    data_json = {
        "target": {"type": "ETF", "code": code, "name": quote.get("name", ""), "index": "中证港股通互联网(931637)"},
        "analysis_time": datetime.now().isoformat(),
        "market_status": "盘中" if weekday < 5 and (9*60+30 <= minutes <= 15*60) else "盘后",
    ...
    print(f"数据已保存: {dir_path}/data.json")
