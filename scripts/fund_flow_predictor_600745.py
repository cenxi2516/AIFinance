...
标的: *ST闻泰 (600745)
日期: 2026-07-13
类型: 上海主板ST个股 → 尝试A轨(资金流), 不可用则降级B轨(量价)
⚠️ ST风险警示: *ST股票存在退市风险, PE为负, 5%涨跌停限制, 投机性极强
"""
...
def fetch_kline_tencent(code: str, days: int = 60) -> list[dict]:
    ...
    klines = []
    for d in raw:
        # 腾讯K线通常6字段[date,open,close,high,low,volume]
        # 但分红除权日可能多第7字段(分红信息dict)，只取前6个
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# V1.4 新增: 概念板块资金流 + 融资融券 (替代不可用的 push2his)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# 个股→概念板块映射（核心概念板块，手工维护 + 搜索API回退）
STOCK_CONCEPT_MAP = {
    "600176": "BK0546",  # 中国巨石 → 玻璃玻纤
    "600745": "BK1031",  # *ST闻泰 → 半导体概念 (闻泰科技旗下安世半导体)
}
...
def fetch_concept_board_flow(code: str) -> dict:
    ...
    数据源: 东财 push2 clist (通过 em_get, 限流可用)
    返回: {board_code, board_name, stocks: [{code, name, change_pct, main_net_yi}], total_main_net_yi}
    """
...
def fetch_margin_data(code: str) -> dict:
    ...
    数据源: 东财 datacenter-web (非 push2his 通道, 可用)
    API: RPTA_WEB_RZRQ_GGMX, filter用 SCODE (非 SECURITY_CODE)
    返回: {available, records: [{date, rzye_yi, rzmre_yi, rzche_yi, net_yi}], trend, latest_balance_yi}
    """
    try:
        ...
        for item in raw_records[:30]:
            ...
            rzye = (item.get('RZYE') or 0) / 1e8     # 融资余额(亿)
            rzmre = (item.get('RZMRE') or 0) / 1e8    # 融资买入额(亿)
            rzche = (item.get('RZCHE') or 0) / 1e8    # 融资偿还额(亿)
            records.append({
                'date': date_str,
...
def detect_selling_exhaustion(daily: list[dict], track: str = "B") -> dict:
    """
    F1: 卖压衰竭检测
    A轨: 近5日主力净流出额逐日递减 + 最新流出<前5日均50% + 不再创新低
...
def detect_institution_return(daily: list[dict], track: str = "B") -> dict:
    """
    F2: 资金回流检测
    A轨: 前期机构(超大单)持续流出 → 近3日机构转为净流入 + 散户仍在流出
    B轨: 前期持续下跌 → 近3日有放量阳线
    """
    if len(daily) < 10:
        return {"triggered": False, "reason": "数据不足"}
...
def detect_retail_exhaustion(daily: list[dict], track: str = "B") -> dict:
    """
    F3: 恐慌出尽检测
    A轨: 散户(中小单)连续5日净流出 + 近3日流出额逐日递减 + 跌幅收窄
    B轨: 近5日跌幅收窄 + 成交量显著萎缩（<前5日均量60%）
    """
    if len(daily) < 5:
        return {"triggered": False, "reason": "数据不足"}
...
def detect_accelerating_selling(daily: list[dict], track: str = "B") -> dict:
    ...
    A轨: 主力连续3日净流出 + 每日流出额递增 + 最新流出>前5日均值2倍
...
def detect_top_distribution(daily: list[dict], track: str = "B") -> dict:
    """
    F5: 高位逆转 —— 典型的顶部信号
    A轨: 近5日股价上涨>3% + 机构(超大单)净流出 + 散户(中小单)净流入
    B轨: 前期(5-10日)涨幅>10% + 近2日出现放量暴跌
    """
    if len(daily) < 10:
        return {"triggered": False, "reason": "数据不足"}
...
def calc_safety_score(safety_signals: dict, danger_signals: dict, daily: list[dict],
    ...
    if track == "A":
        if safety_signals.get("f1_exhaustion", {}).get("triggered"):
            score += 15
            adjustment_details.append(("F1 主力流出衰竭", +15))
        if safety_signals.get("f2_institution_return", {}).get("triggered"):
            score += 12
            adjustment_details.append(("F2 机构试探回流", +12))
        if safety_signals.get("f3_retail_exhaustion", {}).get("triggered"):
            score += 10
            adjustment_details.append(("F3 散户恐慌出尽", +10))
    ...
    if track == "A":
        if danger_signals.get("f4_accelerating", {}).get("triggered"):
            score -= 22
            adjustment_details.append(("F4 主力加速流出", -22))
        if danger_signals.get("f5_top_distribution", {}).get("triggered"):
            score -= 20
            adjustment_details.append(("F5 顶部派发", -20))
    else:
        if danger_signals.get("f4_accelerating", {}).get("triggered"):
            score -= 20
...
def _print_report(report: dict):
    ...
    if daily:
        ...
        for d in daily[-20:]:
            ...
            # 换手率估算: 腾讯K线成交量单位是"手"(1手=100股), 需×100转为股数
...
def main():
    code = "600745"
    target_name = "*ST闻泰"
    print(f"[识别] 上海主板ST个股: {target_name} ({code}) ⚠️ *ST退市风险警示")

    # ━━ 1. 拉取K线 ━━
    print("[1/6] 拉取腾讯日K线(qfq,60日)...")
    ...
    if concept_flow.get("available"):
        ...
        tf = concept_flow.get("target_flow", {})
        if tf:
            print(f"  闻泰排名: {concept_flow.get('target_rank',0)}/{concept_flow.get('target_rank_total',0)} | "
    ...
    report = {
        ...
        "concept_flow": concept_flow,       # V1.4 概念板块资金流
        "margin_data": margin_data,         # V1.4 融资融券
    ...
    # ━━ 保存报告 ━━
    output_dir = f"src/资金预判/{datetime.now().strftime('%Y-%m-%d')}-{target_name.replace('*','')}-{code}-安全评估"
