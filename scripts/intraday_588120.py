#!/usr/bin/env python3
"""
盘中实时交易信号系统 V1.3 — 科创50ETF国泰(588120) 七维分析
日期: 2026-07-21 盘中
数据源: mootdx(分时) + 腾讯(行情/K线) + 东财push2(板块/广度) + 同花顺(北向)
...
# ============================================================
# Layer 2: mootdx 分时 → 买卖力道分析 (ETF版)
# ============================================================
def fetch_minute_data_mootdx(code):
    from mootdx.quotes import Quotes
    client = Quotes.factory(market='std')
    try:
        df = client.minutes(symbol=code)
...
def analyze_minute_flow(minute_data):
    ...
    return {
        ...
        "source": "mootdx 分时数据 → 买卖力道推导 (ETF版)",
    }
...
def fetch_market_breadth():
    ...
    if breadth_fresh.get("status") == "pending":
        up_ratio = 50; limit_up = limit_down = 0  # 默认中性
...
def generate_scenarios(flow, quote, sector, news, nb, ms, mb):
    ...
    a_score += 20 if no_new_low else 0
    ...
    c_score += 30 if high_turn else 0
    ...
    c_score += 25 if sell_accel else 0
    ...
    d_score += 25 if sell_accel_d else 0
    ...
    d_score += 15 if sentiment_cold else 0
    ...
    d_score += 10 if breadth_bear else 0
...
def main():
    ...
    # [2/7] 分时
    print("[2/7] 拉取分时数据(mootdx)...")
    minute_data = fetch_minute_data_mootdx(code)
