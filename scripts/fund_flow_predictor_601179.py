    ...
    if daily:
        ...
        if track == "A":
            ...
            if concept_md and concept_md.get("available"):
                ...
                lines.append(f"")
                lines.append(f"板块: {concept_md.get('board_code','')} | 成分股: {concept_md.get('stock_count',0)}只")
                lines.append(f"板块主力净流合计: {concept_md.get('total_main_net_yi',0):+.2f}亿")
                tf = concept_md.get("target_flow", {})
                if tf:
                    lines.append(f"目标个股: 主力{tf.get('main_net_yi',0):+.2f}亿 | 排名 {concept_md.get('target_rank',0)}/{concept_md.get('target_rank_total',0)}")
                lines.append(f"")
                lines.append(f"| 排名 | 代码 | 名称 | 涨跌幅 | 主力净流(亿) |")
                lines.append(f"|------|------|------|--------|-------------|")
                sorted_md = sorted(concept_md.get("stocks", []), key=lambda x: x['main_net_yi'], reverse=True)
                for i, s in enumerate(sorted_md):
                    marker = " **←目标**" if s['code'] == report['target'] else ""
...
if __name__ == "__main__":
    CODE = "601179"
    print(f"🛡️  fund-flow-predictor V1.4 — 中国西电({CODE})安全评估")
    print(f"  开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print()

    # Step 1: K线
    print("[1/5] 拉取腾讯日K线(qfq)...")
    ...
    print(f"  当前价: {realtime.get('price', 0):.2f} | 名称: {realtime.get('name', '')}")

    # Step 3: 资金流 + 概念板块 + 融资融券 (并行)
    print("[3/5] 拉取辅助数据...")
    fund_flow = fetch_fund_flow(CODE)
    if fund_flow:
        merged = merge_fund_flow(daily, fund_flow)
        print(f"  个股资金流: {merged}日已合并")
    else:
        print(f"  个股资金流: 不可用，将使用B轨")

    concept_flow = fetch_concept_board_flow(CODE)
    if concept_flow.get("available"):
        print(f"  概念板块资金流: {concept_flow.get('board_code','')} ({concept_flow.get('stock_count',0)}只成分股)")
    else:
        print(f"  概念板块资金流: {concept_flow.get('reason','不可取')}")

    margin_data = fetch_margin_data(CODE)
    if margin_data.get("available"):
        print(f"  融资融券: {margin_data.get('latest_balance_yi',0):.2f}亿 | 趋势: {margin_data.get('trend','')}")
    else:
        print(f"  融资融券: {margin_data.get('reason','不可取')}")

    # Step 4: 新闻
    print("[4/5] 拉取新闻...")
    keyword = realtime.get("name", "中国西电")
    news = fetch_news(keyword)
    print(f"  获取 {len(news)} 条新闻")

    # Step 5: 确定信号轨
    track = determine_track(daily)
    track_label = "A轨(资金流+量价)" if track == "A" else "B轨(量价+消息+概念板块代理)"
    print(f"[5/5] 信号轨: {track_label}")
    ...
    report = {
        ...
        "target": CODE,
        "target_type": "stock",
        "target_name": realtime.get("name", "中国西电"),
        "track": track,
    ...
    # 保存报告
    output_dir = f"src/资金预判/{datetime.now().strftime('%Y-%m-%d')}-中国西电-安全评估"
