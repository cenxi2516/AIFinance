...
def fetch_news(code: str) -> list[dict]:
    """获取新闻"""
    try:
        from a_stock_api import eastmoney_stock_news
        raw = eastmoney_stock_news(code, page_size=10)
        news = []
        for item in raw:
            news.append({
...
def detect_selling_exhaustion(daily: list[dict], track: str = "B") -> dict:
    ...
    if track == "A":
        ...
        return {
            ...
            "decreasing": is_decreasing, "latest_vs_avg_ratio": round(ratio, 2), "no_new_low": no_new_low,
        ...
        return {
            ...
            "decreasing_declines": is_decreasing, "volume_shrink": vol_shrink, "no_new_low": no_new_low,
...
def detect_retail_exhaustion(daily: list[dict], track: str = "B") -> dict:
    ...
    if track == "A":
        ...
        return {
            ...
            "consecutive_outflow_days": 5, "decreasing": is_decreasing, "decline_narrowing": decline_narrowing,
            "strength": "强" if triggered else "弱",
...
def detect_volume_stabilization(daily: list[dict], track: str = "B") -> dict:
    ...
    return {
        ...
        "narrow_range": narrow_range, "volume_shrink": volume_shrink, "amp_narrow": amp_narrow,
        "outflow_shrink": outflow_shrink, "avg_vol_20": round(avg_vol_20, 0), "recent_vol_3": round(recent_vol, 0),
        "strength": "强" if triggered else "弱",
...
def detect_accelerating_selling(daily: list[dict], track: str = "B") -> dict:
    ...
    if track == "A":
        ...
        return {
            ...
            "danger_level": "高危", "accelerating": accelerating, "volume_expanding": vol_expanding,
...
def detect_top_distribution(daily: list[dict], track: str = "B") -> dict:
    ...
    if track == "A":
        ...
        return {
            ...
            "danger_level": "高危", "early_rise_pct": round(early_rise, 1), "has_distribution": has_distribution,
...
def check_stabilization(daily: list[dict], track: str = "B") -> dict:
    ...
    return {
        "is_stabilized": is_stabilized, "passed_count": passed, "total_count": 5, "checks": checks,
...
def calc_safety_score(safety_signals: dict, danger_signals: dict, daily: list[dict],
    ...
    if margin_data and margin_data.get("available"):
        ...
        if trend == "下降" and net_5d < -1:
            score -= 7; adjustment_details.append((f"融资去杠杆(近5日{net_5d:+.1f}亿)", -7))
        elif trend == "下降" and net_5d < 0:
            score -= 4; adjustment_details.append((f"融资温和下降(近5日{net_5d:+.1f}亿)", -4))
        elif trend == "上升" and net_5d > 2:
            score += 5; adjustment_details.append((f"融资加杠杆(近5日+{net_5d:.1f}亿)", +5))
        elif trend == "上升" and net_5d > 0:
            score += 3; adjustment_details.append((f"融资温和上升(近5日+{net_5d:.1f}亿)", +3))
    ...
    if concept_flow and concept_flow.get("available") and track == "B":
        ...
        if total_main < -5:
            score -= 5; adjustment_details.append((f"概念板块主力大幅流出({total_main:+.1f}亿)", -5))
        if target_main > 0 and concept_flow.get("target_rank", 99) == 1:
            score += 4; adjustment_details.append(("概念板块龙头+主力流入", +4))
    ...
    return {
        "safety_score": score, "safety_level": level, "can_enter": can_enter, "description": desc,
...
def _save_markdown(report: dict, path: str):
    ...
    if margin_md and margin_md.get("available"):
        ...
        lines.append(f"**近5日净买卖**: {margin_md.get('net_flow_5d_yi', 0):+.2f}亿 | **近10日净买卖**: {margin_md.get('net_flow_10d_yi', 0):+.2f}亿")
        lines.append(f"")
...
def main():
    ...
    target_name = "东方电气"
    target_type = "stock"
    print(f"[识别] {target_type}: {target} ({target_name})")
    ...
    fund_flow = fetch_fund_flow(target)
    if fund_flow:
        merged = merge_fund_flow(daily, fund_flow)
        print(f"  合并 {merged} 日资金流")
    ...
    print("[5/6] 拉取辅助数据...")
    concept_flow = fetch_concept_board_flow(target)
    print(f"  概念板块: {concept_flow.get('board_code', '未找到')} {'可用' if concept_flow.get('available') else '不可用'}")
    margin_data = fetch_margin_data(target)
    print(f"  融资融券: {'可用' if margin_data.get('available') else '不可用'}")

    # 2. 选择信号轨
    track = determine_track(daily)
    print(f"[6/6] 信号轨: {'A轨(资金流+量价)' if track == 'A' else 'B轨(量价+消息面)'}")
    ...
    report = {
        ...
        "target_name": realtime.get("name", target_name),
        "track": track,
        "track_label": "A轨(资金流+量价)" if track == "A" else "B轨(量价+消息面)",
        "summary": {
