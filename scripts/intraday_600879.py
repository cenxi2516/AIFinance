#!/usr/bin/env python3
"""
盘中实时交易信号系统 V1.3 — 航天电子(600879) 七维分析
日期: 2026-07-13 盘中
...
def fetch_aerospace_sector_sentiment():
    """拉取全量概念板块 → 筛选航天/军工核心板块"""
    try:
    ...
    # 筛选航天/军工板块
    sentiment_blocks = []
    for bk_code, bk_name in AEROSPACE_SECTORS.items():
...
def generate_scenarios(quote, flow, sector, news, parties, divergence, nb, ms, mb):
    ...
    b_prob = round(b_score / 125 * 100, 1) if not no_news else round(b_score / 115 * 100, 1)
    ...
    c_conditions.append({"condition": "已涨>3%且资金流转向", "threshold": "涨>3%+资金减缓/流出",
                         ...
                         "met": high_and_turning, "weight": 30})
    ...
    d_conditions.append({"condition": "有利空消息", "threshold": "利空>0条",
                         "actual": f"利空{news_bearish}条" if bearish_news else "无",
                         "met": bearish_news, "weight": 15})
...
def print_report(quote, flow, mootdx_flow, sector, news, nb, ms, mb, scenarios, signals):
    ...
    print()
    print("═" * 70)
    print(f"  📊 盘中实时交易信号 V1.3: 航天电子(600879)")
    print(f"  分析时间: {DATE_TIME_STR}")
    print(f"  时效性阶段: {FRESHNESS['phase_name']}")
    print(f"  数据可用: 行情✅ | 资金流✅ | {'板块✅' if FRESHNESS['freshness']['sector']['status']=='ready' else '板块⚠️延迟'} | {'北向✅' if FRESHNESS['freshness']['north_bound']['status']=='ready' else '北向⚠️延迟'} | 大盘✅ | {'广度✅' if FRESHNESS['freshness']['breadth']['status']=='ready' else '广度⚠️延迟'}")
    print("═" * 70)
...
def main():
    print(f"\n{'='*60}")
    print(f"  航天电子(600879) 盘中实时交易信号 V1.3")
    ...
    # 3. 板块情绪
    print("\n[3/6] 拉取航天/军工板块情绪...")
    sector = fetch_aerospace_sector_sentiment()
    if "error" in sector:
...
def _save_report(quote, flow, mootdx_flow, sector, news, nb, ms, mb, scenarios, signals):
    ...
    if freshness.get("downgraded_count", 0) > 0:
        ...
        a(f"降权条件数: {freshness.get('downgraded_count', 0)}\n\n")
        for detail in freshness.get("downgrade_details", []):
