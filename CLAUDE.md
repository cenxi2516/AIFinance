...
- **数据真实性铁律**：==报告生成前，所有行情数据必须经过 ≥2 个权威平台交叉验证，差异 <1% 方可通过。单源数据必须标注 ⚠️ 风险。不交叉验证不写入报告。==
...
### 行情数据多平台交叉验证（所有 skill 强制执行）

> **铁律**: ==数据不交叉验证，不写入报告。==

每个 skill 在生成报告前，必须对行情数据执行多平台交叉验证。统一使用公共模块 `.claude/skills/_shared/cross_validation.py`。

**数据源矩阵**：

| 维度 | 主源 | 验证源 | 验证方式 |
|------|------|--------|---------|
| 价格/量/额 | 腾讯 `qt.gtimg.cn` | 新浪 `hq.sinajs.cn` | 双源对比，差异 < 0.5% |
| K线 | 腾讯 `qfqday` | — | 单源，但 vs 行情价交叉验证 |
| PE/PB/市值 | 腾讯 | — | 单源（新浪不提供），标注 ⚠️ |
| 个股资金流 | 东财 `push2his` | — | 单源，不可用时降级为量价分析 |
| 概念板块资金流 | 东财 `push2 clist` | — | 单源，不可用时用行业资金流代理 |
| 融资融券 | 东财 `datacenter-web` | — | 单源 |
| 新闻 | 东财 `search-api-web` | — | 单源 |
...
# 在 Console 报告 Part 1 中打印验证结果
print_validation_summary(realtime.get("_validation", {}))

# 在 Markdown 报告中添加验证结果
markdown_lines = markdown_validation_summary(realtime.get("_validation", {}))
```

**质量评级**：
- 🟢 高置信度：全部双源验证通过 → 可正常写入报告
- 🟡 中等置信度：部分单源但无不一致 → 可写入，标注"单源"
- 🔴 低置信度：≥3 项不一致 → **不可写入报告**，需排查数据源

**skill 开发规范**：
- 新 skill 创建时必须集成 `cross_validation` 模块
- 既有 skill 逐步改造，优先改造涉及实时行情的 skill
- 仅有单一来源的数据维度，必须在报告中明确标注"⚠️ 单源，未经交叉验证"
...
a-stock-data（数据获取层）
    ...
    ├── tech-event-analyzer（科技事件影响评分层） → "这个消息对科技冲击有多大"
    ...
    ├── commodity-futures-tracker（大宗商品期货追踪层） → "商品涨了还是跌了、为什么、A股怎么映射"
...
| 维度 | a-stock-data | policy-event-tracker | tech-event-analyzer | serenity-skill | moat-hunter | cross-sector-ma-tracker | old-guard-stocks | earnings-tracker | capital-flow-tracker | etf-fund-flow-tracker | limit-up-tracker | industry-sentiment-tracker | fund-flow-predictor | commodity-futures-tracker | hard-tech-dashboard | stock-pick |
|------|-------------|---------------------|--------------------|---------------|-------------|------------------------|-----------------|-----------------|---------------------|----------------------|------------------|---------------------------|--------------------|--------------------------|--------------------|------------|
| 职责 | 数据获取与清洗 | 政策事件追踪与影响分析 | 全球科技事件搜索+五维量化评分+共振检测+事件衰减追踪 | 产业链基本面分析 | 企业不可替代性验证 | 传统企业跨界科技并购追踪 | 质量价值筛选+错杀识别 | 业绩预告+财务指标+拐点识别+超预期分析 | 资金流向监控+个股涨跌归因 | ETF资金流+风格判断+盘面分析 | 涨停板复盘+板块情绪分析 | 行业情绪聚合+五维加权+趋势追踪 | 买入安全区间研判+企稳确认+买卖时机参考 | 全球期货价格追踪+供需分析+趋势研判+A股传导映射 | 六维合成研判(外围+政策+产业链+资金+情绪+业绩) | 多策略选股筛选 |
| 输入 | 股票代码 / 查询条件 | 时间窗口 / 行业关键词 | 全球科技事件(中美韩日欧) × 7大科技子赛道 | 产业链知识 + 财报/公告 | 目标公司 + 产业链位置 | 时间窗口 / 传统行业范围 / 科技方向 | 行业范围 + 财报/估值数据 | 股票代码 / 报告期 / 行业 | 概念板块/行业板块/龙虎榜 | 宽基/行业/主题ETF代码 | 日期 / 市场板块 | 行业→概念板块映射表 | 股票代码/概念板块/ETF + 30天盘面数据 | 26个国际+国内期货品种代码 + 供需/政策/事件搜索 | 8个skill输出 + 外围股市 + 全球政策 | 全市场/板块行情数据 |
| 输出 | 结构化行情/估值/资金/研报/公告数据 | 按重要性排序的事件+行业/标的映射 | 0-10影响分+方向标签+传导链+共振矩阵+风险提示+核心标的清单+事件衰减状态 | 稀缺层判断 + 标的优先级 | 不可替代性评级(S/A/B/C) + 供需验证 | S/A/B/C四级跨界信号清单 + 重点案例深度分析 | 老登股清单(核心/优质/观察) + 错杀程度评分 | 业绩预告全类型+财务序列+拐点信号+超预期幅度 | 板块资金流排名+三方资金分类+个股归因 | ETF全景报告+风格判断+行业轮动+多维交叉验证 | 涨停全景报告+行业/概念排名+炸板分析+封板强度+连板追踪 | 三级情绪指数(全市场/行业/细分方向)+热力图+信号检测 | 0-100安全评分+企稳确认清单+买入/离场参考条件+15条可验证规则 | 全品种价格仪表盘+涨跌排名+内外价差+供需评分卡+事件分析+A股映射信号 | 六维雷达+综合评分(1-10)+传导路径+子板块优先级+风险矩阵 | 通过/拒绝分层清单 |
| **quant-dashboard** | **全市场量化研判(五层引擎+动态权重+30+条历史规律)** | **全部下层skill输出+实时行情+ETF+消息** | **科技事件冲击信号(影响分+共振方向)** | — | — | — | — | — | — | — | — | — | — | **商品价格信号(大类资产轮动输入)** | — | — |
...
- **tech-event-analyzer** ★NEW：全球科技事件影响分析器——搜索中美韩日欧科技事件×7大子赛道(半导体/AI算力/消费电子/高端制造/新材料/航空航天/通信量子)，五维度量化评分(市场情绪冲击30%+产业链影响25%+时效20%+确定性15%+新颖度10%)，输出0-10影响分+传导链+共振检测+利好出尽风险检查+事件生命周期衰减追踪。核心价值：把"台积电上调资本开支""Kimi算力紧缺"等科技新闻转化为可比较的量化冲击分，直接回答"这个消息对A股科技冲击有多大"。触发词："科技事件""科技消息""科技新闻 影响""对A股影响""科技 催化""算力/半导体/AI 消息面"。"台积电/news/事件 对A股影响""科技赛道 事件扫描"。详见 `.claude/skills/tech-event-analyzer/SKILL.md`
...
- **hard-tech-dashboard**：A股硬科技走势综合研判仪表盘——整合六维数据(外围股市映射/全球政策事件/产业链趋势/A股资金面/A股情绪面/业绩验证)，可消费 tech-event-analyzer 的事件影响评分。输出硬科技板块全景研判报告（综合评分1-10分 + 5条传导路径 + 5大子板块优先级排序 + 风险矩阵）。触发词："硬科技""科技板块走势""半导体/AI/芯片 行情研判""国产替代 机会""科技股 还能不能涨""全球科技 对A股影响""费城半导体/纳指 对A股科技影响""科技赛道 配置"。详见 `.claude/skills/hard-tech-dashboard/SKILL.md`
...
- **commodity-futures-tracker** ★NEW：全球大宗商品期货追踪器——覆盖黄金/白银/稀土/工业金属/有色金属/能源金属/战争金属/工业材料等 26 个国际+国内期货品种，新浪 `hq.sinajs.cn` 免费直连拉取实时价格+涨跌幅。四层分析引擎(价格仪表盘→供需基本面→政策事件驱动→A股传导映射)，输出全品种价格仪表盘+涨跌排名+内外价差+供需评分卡+事件分析+10条商品价格历史规律。核心价值：回答"商品涨了还是跌了？为什么涨跌？还会继续吗？A股怎么映射？"四个问题，填补非股票资产类别的空白。触发词："期货""大宗商品""黄金价格""白银走势""铜价""铝价""稀土""碳酸锂""工业金属""COMEX""LME""沪金""沪银""沪铜""供需关系""战争金属"。详见 `.claude/skills/commodity-futures-tracker/SKILL.md`
...
Phase 5: 候选筛选 + 估值分析（comparison.md，用 a-stock-data 拉取实时数据）
    ↓
Phase 6: 情景推演 + 时机参考（scenario-analysis.md，整合基本面+资金面+情绪面+ETF资金流，调用 capital-flow-tracker + etf-fund-flow-tracker + industry-sentiment-tracker；若涉及资源/有色/能源方向，调用 commodity-futures-tracker 分析大宗商品价格趋势；若聚焦硬科技方向，调用 tech-event-analyzer 扫描全球科技事件 + hard-tech-dashboard 进行六维综合研判）
    ↓
...
│   ├── SKILL.md
│   └── references/
├── tech-event-analyzer/          # 全球科技事件影响分析 ★NEW
│   ├── SKILL.md
│   └── references/
│       ├── tech-universe.md      # 7大子板块×核心标的×BK码映射
│       └── event-state.json      # 事件生命周期追踪状态
...
├── commodity-futures-tracker/    # 大宗商品期货追踪 ★NEW
│   ├── SKILL.md
│   └── references/
│       ├── commodity-universe.md      # 26品种代码全集+A股映射
│       ├── supply-demand-sources.md   # 供需数据权威来源
│       └── historical-patterns.md     # 10条商品价格历史规律
